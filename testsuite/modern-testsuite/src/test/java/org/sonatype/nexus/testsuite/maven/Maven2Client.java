...
{
  public Maven2Client(final HttpClient httpClient,
  ...
  public HttpResponse getIfNewer(String path, Date date) throws IOException {
    ...
    get.addHeader(HttpHeaders.IF_MODIFIED_SINCE, DateUtil.formatDate(date));
    return execute(get);
  }
  ...
  public HttpResponse putIfUmmodified(String path, Date date, HttpEntity entity) throws IOException {
    final URI uri = resolve(path);
    final HttpPut put = new HttpPut(uri);
    put.addHeader(HttpHeaders.IF_UNMODIFIED_SINCE, DateUtil.formatDate(date));
    put.setEntity(entity);
    return execute(put);
  }
