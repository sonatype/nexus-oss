...
public class UrlPathParser {
  ...
  public UrlPathInterpretation interprete(Request request) throws ResourceException {
    ...
    if (YUM_REPOSITORY_DIR_NAME.equals(lastSegments.get(FIRST_PARAM))) {
      if (lastSegments.size() == 1) {
        return new UrlPathInterpretation(repoUrl, YUM_REPOSITORY_DIR_NAME, true, true, pathToIndex(segments));
      }

      if (lastSegments.get(SECOND_PARAM).length() == 0) {
        return new UrlPathInterpretation(repoUrl, YUM_REPOSITORY_DIR_NAME, true);
      }
