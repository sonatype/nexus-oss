    ...
    extends AbstractNexusTestBase
{
    ...
    private void addMemberRepo( String memberRepo )
        throws Exception
    {
        ...
        repoXml =
            repoXml.replace(
                "</repositories>",
                format(
                    "<repo-group-member><id>%s</id><name>%s</name><resourceURI>%s</resourceURI></repo-group-member></repositories>",
                    memberRepo, memberRepo, GROUP_REPO_URL + "/" + memberRepo ) );
        consume( executePut( GROUP_REPO_URL, new StringEntity( repoXml ), CONTENT_XML ).getEntity() );
    }

    private void removeMemberRepo( String memberRepo )
        throws Exception
    {
    ...
    private String removeMemberRepoFromXml( String memberRepo )
        throws Exception
    {
    ...
    private String getGroupRepoPrimaryXml()
        throws IOException, AuthenticationException
    {
        return gzipResponseContent( executeGetWithResponse( NEXUS_BASE_URL + "/content/groups/" + GROUP_REPO_ID
            + "/repodata/primary.xml.gz" ) );
    }

    private void givenGroupRepoWith2Rpms()
        throws Exception, AuthenticationException, UnsupportedEncodingException, IOException, ClientProtocolException,
