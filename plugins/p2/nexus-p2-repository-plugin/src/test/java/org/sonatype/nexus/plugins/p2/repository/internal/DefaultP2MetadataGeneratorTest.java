...
public class DefaultP2MetadataGeneratorTest {
	...
	Logger logger;
	@InjectMocks
	DefaultP2MetadataGenerator generator = new DefaultP2MetadataGenerator(repositories, mimeSupport, artifactRepository, metadataRepository, publisher);
	...
	public void setup() throws IOException {
	    ...
	    doAnswer(ans).when(artifactRepository).write(any(URI.class), any(Collection.class), any(String.class), any(Map.class), any(String[][].class));
	    doAnswer(ans).when(metadataRepository).write(any(URI.class), any(Collection.class), any(String.class), any(Map.class));
	...
	public void Test_NEXUS_5995() throws IOException, NoSuchRepositoryException {
		...
		when(publisher.generateFeatureIUs(any(Boolean.class), any(Boolean.class), any(File[].class))).thenReturn(Arrays.asList(jarUnit, groupUnit));
