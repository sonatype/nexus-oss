 ...
 * @date 13. March 2008
 * @license Ext.ux.form.UploadPanel is licensed under the terms of the Open
...
Ext.ux.UploadPanel = Ext.extend(Ext.Panel, {
  ...
  /**
   * @cfg {String} buttonsAt Where buttons are placed. Valid values are tbar,
   *      bbar, body (defaults to 'tbar')
   */
  ,
  buttonsAt : 'tbar'
  ...
  initComponent : function() {
    ...
    Ext.apply(this, {
          items : [{
            ...
            tpl : this.tpl
                || new Ext.XTemplate('<tpl for=".">' + '<div class="ux-up-item">'
                        // + '<div class="ux-up-indicator">&#160;</div>'
                        + '<div class="ux-up-icon-file {fileCls}">&#160;</div>' + '<div class="ux-up-text x-unselectable" qtip="{fileName}">{shortName}</div>' + '<div id="remove-{[values.input.id]}" class="ux-up-icon-state ux-up-icon-{state}"'
                        + 'qtip="{[this.scope.getQtip(values)]}">&#160;</div>' + '</div>' + '</tpl>', {
                      scope : this
                    }),
            listeners : {
              click : {
    ...
    this.addEvents(
        /**
         * Fires before the file is added to store. Return false to cancel the
         * add
         * 
         * @event beforefileadd
         * @param {Ext.ux.UploadPanel}
         *          this
         * @param {Ext.Element}
         ...
         *          Record (containing the input) that has been added to the
         *          store
         */
        , 'fileadd'
        /**
         * Fires before the file is removed from the store. Return false to
         * cancel the remove
         * 
         ...
         *          store
         * @param {Ext.data.Record}
         *          Record (containing the input) that is being removed from the
         *          store
         */
        , 'beforefileremove'
        /**
        ...
        , 'fileremove'
        /**
         * Fires before all files are removed from the store (queue). Return
         * false to cancel the clear. Events for individual files being removed
         * are suspended while clearing the queue.
         * 
        ...
        , 'beforequeueclear'
        /**
         * Fires after the store (queue) has been cleared Events for individual
         * files being removed are suspended while clearing the queue.
         * 
         * @event queueclear
         * @param {Ext.ux.UploadPanel}
         *          this
        ...
        , 'queueclear'
        /**
         * Fires after the upload button is clicked but before any upload is
    ...
    this.relayEvents(this.uploader, ['beforeallstart', 'allfinished', 'progress']);
