 ...
 * @date 15. March 2008
 * @license Ext.ux.FileUploader is licensed under the terms of the Open Source
...
Ext.ux.FileUploader = function(config) {
  ...
  this.addEvents(
      /**
       * @event beforeallstart Fires before an upload (of all files) is started.
      ...
      , 'allfinished'
      /**
       * @event beforefilestart Fires before the file upload is started. Return
       *        false to cancel the event. Fires only when singleUpload = false
       * @param {Ext.ux.FileUploader}
       *          this
      ...
      , 'beforefilestart'
      /**
       * @event filefinished Fires when file finished uploading. Fires only when
       *        singleUpload = false
       * @param {Ext.ux.FileUploader}
       *          this
       * @param {Ext.data.Record}
...
Ext.extend(Ext.ux.FileUploader, Ext.util.Observable, {
   ...
   *      false to start the next request only if previous one has been
   *      completed (or failed). Applicable only if singleUpload = false
   */
  ,
  concurrent : true
  ...
  /**
   * @cfg {Number} progressInterval How often (in ms) is progress requested from
   *      server
   */
  ,
   ...
   * 
   * @param {Ext.data.Record}
   *          record Optional, if not set singleUpload = true is assumed and the
   *          global stop is initiated
   */
  ,
   ...
   * 
   * @private but dispatches processing to processSuccess and processFailure
   *          functions
   */
  ,
