 ...
 * @extends Ext.Button Ext.Button that provides a customizable file browse
 *          button. Clicking this button, pops up a file dialog box for a user
 *          to select the file to upload. This is accomplished by having a
 *          transparent <input type="file"> box above the Ext.Button. When a
 *          user thinks he or she is clicking the Ext.Button, they're actually
 *          clicking the hidden input "Browse..." box. Note: this class can be
 *          instantiated explicitly or with xtypes anywhere a regular Ext.Button
 *          can be except in 2 scenarios: - Panel.addButton method both as an
 *          instantiated object or as an xtype config object. - Panel.buttons
 *          config object as an xtype config object. These scenarios fail
 *          because Ext explicitly creates an Ext.Button in these cases. Browser
 *          compatibility: Internet Explorer 6: - no issues Internet Explorer 7: -
 *          no issues Firefox 2 - Windows: - pointer cursor doesn't display when
 *          hovering over the button. Safari 3 - Windows: - no issues.
 * @author loeppky - based on the work done by MaximGB in Ext.ux.UploadDialog
 *         (http://extjs.com/forum/showthread.php?t=21558) The follow the curosr
 *         float div idea also came from MaximGB.
...
Ext.ux.form.BrowseButton = Ext.extend(Ext.Button, {
       ...
       *      effectively it covers the Ext.Button. In addition, clipEl is given
       *      a green background and floatEl a red background to see how well
       *      they are positioned.
       */
      debug : false,
       ...
       *       the width of the IE "Browse" button's width (65 pixels), since IE
       *       doesn't let you resize it. We define this width so we can quickly
       *       center floatEl at the mouse cursor without having to make any
       *       function calls.
       * @private
       */
      FLOAT_EL_WIDTH : 60,
       ...
       *       the height of the "Browse" button's height. We define this height
       *       so we can quickly center floatEl at the mouse cursor without
       *       having to make any function calls.
       * @private
       */
      FLOAT_EL_HEIGHT : 18,
      ...
      /**
       * @property buttonCt
       * @type Ext.Element Element that contains the actual Button DOM element.
       *       We store a reference to it, so we can easily grab its size for
       *       sizing the clipEl.
       * @private
       */
      buttonCt : null,
       ...
       *       positioned to fill the area of Ext.Button and has overflow turned
       *       off. This keeps floadEl tight to the Ext.Button, and prevents it
       ...
       *       construction using the "handler" config option. We need to null
       *       out the "handler" property so that it is only called when a file
       *       is selected.
       * @private
       */
      ...
      initComponent : function() {
        ...
        // them.
        // This is done so that this class can control when the handler is
        // called.
        // There are some cases where the hidden file input browse button
        // doesn't completely cover the Ext.Button.
        // The handler shouldn't be called in these cases. It should only be
        // called if a new file is selected on the file system.
      ...
      onRender : function(ct, position) {
        Ext.ux.form.BrowseButton.superclass.onRender.call(this, ct, position); // render
                                                                                // the
                                                                                // Ext.Button
        ...
        /** SONTYPE DIFF * */
        /***********************************************************************
         * Original if ( this.debug ) { this.clipEl.applyStyles({
         * 'background-color': 'green' }); this.floatEl.applyStyles({
         * 'background-color': 'red' }); } else { this.clipEl.setOpacity(0.0); }
         **********************************************************************/
        /** Fixed * */
       ...
       * {@link #resize} event. This method can accept either width and height
       * as separate numeric arguments, or you can pass a size object like
       * {width:10, height:20}.
       * 
       * @param {Number/Object}
       *          width The new width to set, or a size object in the format
       *          {width, height}
       * @param {Number}
       *          height The new height to set (not required if a size object is
      ...
      setSize : function(w, h) {
        ...
        { // this code is nasty but performs better with floaters
      ...
      /**
       * Creates the input file element and adds it to inputFileCt. The created
       * input file elementis sized, positioned, and styled appropriately. Event
       * handlers for the element are set up, and a tooltip is applied if
