...
Sonatype.SearchStore = function(config) {
  var config = config || {};
  var defaultConfig = {
    searchUrl : Sonatype.config.servicePath + '/data_index1'
  };
...
Ext.extend(Sonatype.repoServer.SearchResultGrid, Ext.grid.GridPanel, {
      formatVersionLink : function(value, p, record, rowIndex, colIndex, store) {
        ...
        return '<a href="#nexus-search;gav~' + gid + '~' + aid + '~' + ver + '~~~kw,versionexpand' + ' " onmousedown="cancel_bubble(event)" onclick="cancel_bubble(event); return true;">Drill Down</a>';
      },
