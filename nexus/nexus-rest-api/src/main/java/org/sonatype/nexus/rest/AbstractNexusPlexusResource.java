...
public abstract class AbstractNexusPlexusResource extends
    ...
    protected Nexus getNexusInstance(Request request) throws ResourceException {
        if (NEXUS_INSTANCE_LOCAL.equals(request.getAttributes().get(
                NEXUS_INSTANCE_KEY))) {
            return nexus;
        } else {
            throw new ResourceException(Status.CLIENT_ERROR_NOT_FOUND,
                    "Nexus instance named '"
                            + request.getAttributes().get(NEXUS_INSTANCE_KEY)
                            + "' not found!");
        }
    }

    protected Reference createChildReference(Request request, String childPath) {
        return new Reference(request.getResourceRef()).addSegment(childPath)
                .getTargetRef();
    }

    protected Reference createRootReference(Request request, String relPart) {
        Reference ref = new Reference(request.getRootRef().getParentRef(),
                relPart);
