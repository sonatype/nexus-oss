  ...
  public List<FileTask> getOverlays() {
    ...
    if (!getFeatures().isEmpty()) {
      ...
      try {
        ...
        testFeaturesRepositories = Properties2.load(testFeaturesCfg).getProperty("featuresRepositories");
      }
      ...
      overlays.add(
          fileTaskBuilder.replace()
              ...
              .replace("featuresRepositories = ", "featuresRepositories = " + testFeaturesRepositories + ",")
      );

      overlays.add(
          fileTaskBuilder.replace()
              .inFile(path("nexus/etc/org.apache.karaf.features.cfg"))
              .replace("-edition,", "-edition," + Joiner.on(',').join(getFeatures()) + ",")
      );
    }
