    ...
    implements RecordStoreSession
{
  ...
  public List<Record> find(final RecordQuery query)
  {
    ...
    String queryText = new OrientQueryBuilder(
        ...
        recordIdObfuscator)
            .build();
    List<ODocument> documents = db.query(new OSQLSynchQuery<ODocument>(queryText), getOrientFilter(query.getFilter()));
  ...
  public long count(final RecordQuery query) {
    ...
    String queryText = new OrientQueryBuilder(
        ...
        recordIdObfuscator)
            .withCount(true)
            .build();
    List<ODocument> results = db.query(new OSQLSynchQuery<>(queryText), getOrientFilter(query.getFilter()));
  ...
  private Map<String, Object> getOrientFilter(Map<String, Object> filter) {
    Map<String, Object> orientFilter = Maps.newHashMap();
    for (String key: filter.keySet()) {
      orientFilter.put(key, getOrientObject(filter.get(key)));
    }
    return orientFilter;
  }

  private Object getOrientObject(Object object) {
    if (object instanceof RecordId) {
      return getRid((RecordId) object);
    }
    return object;
  }
