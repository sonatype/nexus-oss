...
@Named
@Singleton
public class JerseyYumGroupRepositoryFactory
    extends JerseyGroupRepositoryFactory
{
    ...
    public int canAdapt( final RepositoryBaseResource resource )
    {
        ...
        if ( score > 0 )
        {
            if ( JerseyYumGroupRepository.PROVIDER.equals( resource.getProvider() ) )
            {
                score++;
            }
        }
    ...
    @Override
    public JerseyYumGroupRepository adapt( final JerseyNexusClient nexusClient,
                                           final RepositoryBaseResource resource )
    {
        return new JerseyYumGroupRepository( nexusClient, (RepositoryGroupResource) resource );
    }

    @Override
    public boolean canCreate( final Class<? extends Repository> type )
    {
        return YumGroupRepository.class.equals( type );
    }

    @Override
    public JerseyYumGroupRepository create( final JerseyNexusClient nexusClient, final String id )
    {
        return new JerseyYumGroupRepository( nexusClient, id );
    }
